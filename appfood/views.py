...
@login_required
def home(request):
    recipes = Recipe.objects.all().select_related('author', 'author__userprofile').prefetch_related('images', 'classify')
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
    recipeimage = RecipeImage.objects.all()
...
#                     sender=request.user,
#                     recipess=recipe,
#                     message=f'{request.user.username} đã bình luận về bài viết của bạn.'
#                 )
...
def savefood(request):
    if request.user.is_authenticated:
        ...
        recipeimage = RecipeImage.objects.all()
        user_profile, created = UserProfile.objects.get_or_create(user=request.user)
        return render(request, 'appfood/savefood.html', {'liked_recipes': liked_recipes,'recipes': recipes,'recipeimage':recipeimage,'user_profile': user_profile})
...
def search_title(request):
    ...
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
    return render(request, 'appfood/search_title.html', {'recipe':recipe,'searched': searched, 'checks': checks,'user_profile': user_profile})
...
def vegetarian(request):
    ...
    vegetarian_recipes = Recipe.objects.filter(classify=vegetarian_class).select_related('author', 'author__userprofile')
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
...
def list_user(request):
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)
    users = User.objects.exclude(id=request.user.id).exclude(is_superuser=True)# Loại Bỏ Admin và Bản Thân để dùng chức năng follow
    following = Follow.objects.filter(follower=request.user).values_list('followed_id', flat=True) 
    return render(request, 'user/list_user.html', {'users': users, 'following': following,'user_profile': user_profile})
...
def search_user(request):
    if request.method == 'POST':
        ...
        following = Follow.objects.filter(follower=request.user).values_list('followed_id', flat=True)
        user_profile, created = UserProfile.objects.get_or_create(user=request.user)
        return render(request, 'user/search_user.html', {'users': users, 'query': query, 'following': following, 'user_profile': user_profile})
    return render(request, 'user/search_user.html', {'users': User.objects.none(), 'query': '', 'following': [], 'user_profile': user_profile})
